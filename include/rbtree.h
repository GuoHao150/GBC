...
int rbtree_minimum(RBRoot *root, int *val) {
  ...
  if (root) node = minimum(root->node);
...
int rbtree_maximum(RBRoot *root, int *val) {
  ...
  if (root) node = maximum(root->node);
...
static void rbtree_insert(RBRoot *root, Node *node) {
  ...
  if (y != NULL) {
    if (node->key < y->key)
      y->left = node;  // 情况2：若“node所包含的值” <
                       // “y所包含的值”，则将node设为“y的左孩子”
    else
      y->right = node;  // 情况3：(“node所包含的值” >=
 ...
 *     right 是右孩子。
 */
static Node *create_rbtree_node(Type key, Node *parent, Node *left,
                                Node *right) {
  Node *p;
...
int insert_rbtree(RBRoot *root, Type key) {
  ...
  // 如果新建结点失败，则返回。
  if ((node = create_rbtree_node(key, NULL, NULL, NULL)) == NULL) return -1;
